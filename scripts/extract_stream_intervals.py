#!/usr/bin/env python3
"""Extract browser stream inter-packet intervals from exported trigger logs."""
...
def log_entries(payload: Any) -> tuple[str | None, list[dict[str, Any]]]:
    if isinstance(payload, dict):
        schema = payload.get("schema")
        entries = payload.get("entries")
        if isinstance(entries, list):
            return schema, [item for item in entries if isinstance(item, dict)]
...
def extract_rows(path: Path, include_null_intervals: bool = False) -> list[dict[str, Any]]:
    ...
    for json_file in iter_json_files(path):
        ...
        schema, entries = log_entries(payload)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract browser stream inter-packet intervals from trigger log JSON exports."
    )
    parser.add_argument("input", type=Path, help="Exported trigger log JSON file or folder.")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
